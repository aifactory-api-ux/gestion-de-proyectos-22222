...
- Name: buttons.spec.ts >> Botones de NotificationList >> botón Mark as Read existe y es clickeable
- Location: tests-functional/buttons.spec.ts:77:3
...
Locator: getByRole('button', { name: /mark as read/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /mark as read/i }).first()
